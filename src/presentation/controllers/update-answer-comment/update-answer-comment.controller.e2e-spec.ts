...
describe('UpdateAnswerCommentController (e2e)', () => {
  ...
  it('should update answer comment and return 200', async () => {
    ...
    const createCommentResponse = await request(app.getHttpServer())
      ...
      .send({ answerId, content: 'Original comment' })
  ...
  it('should return 404 when comment does not exist', async () => {
    ...
    const response = await request(app.getHttpServer())
      .put('/answers/comments/non-existent-id')
      .set('Authorization', `Bearer ${token}`)
      .send({ content: 'Updated comment' })

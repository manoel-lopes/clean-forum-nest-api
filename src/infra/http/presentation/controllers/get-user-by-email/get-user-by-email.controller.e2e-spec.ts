...
describe('GetUserByEmail', () => {
  ...
  it('should return 401 when no token is provided', async () => {
    const response = await getUserByEmail(app, '', { email: 'test@example.com' })
  ...
  it('should return 401 when invalid token is provided', async () => {
    const response = await getUserByEmail(app, 'invalid-token', { email: 'test@example.com' })
  ...
  it('should return 401 when token is expired', async () => {
    ...
    const response = await getUserByEmail(app, expiredToken, { email: 'test@example.com' })
